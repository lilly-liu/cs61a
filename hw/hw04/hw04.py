...
def replace_leaf(t, old, replacement):
    ...
    if is_leaf(t):
        ...
        for b in branches(t):
            list.append(replace_leaf(b, old, replacement))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, user_pass):
        ...
        nonlocal balance
        while len(incorrect_pass) < 3:
            if user_pass != password:
                incorrect_pass.append(user_pass)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(old) == str:#if its an error
        return old
    def joint(amount, user_pass):
