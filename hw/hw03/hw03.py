...
def pingpong(n):
    ...
    def helper(k, num, direction):
        if k == n:
            return num
        else:
            if direction == 1:
                if k % 7 == 0 or num_sevens(k) > 0:
                    return helper(k + 1, num - direction, -direction)
                else:
                    return helper(k + 1, num + direction, direction)
            else:
                if k % 7 == 0 or num_sevens(k) > 0:
...
def count_change(total):
    ...
    def power(num, x): #biggest power below num
        if pow(2, x) <= num:
            return 1 + power(num, x+1)
        else:
            return 0
    return count_partitions(total, power(total, 0))
