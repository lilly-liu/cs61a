...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        op = expr.rest
        list = op.map(lambda x: scheme_eval(x, env)) #scheme list of arguments
        return scheme_apply(proc, list, env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            new_frame.define(formals.first, vals.first) #binds
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[func_name] = LambdaProcedure(formals, body, env)
        return func_name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        let_val = scheme_eval(binding.rest.first, env)
        values = Pair(let_val, values)
