...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place is not beehive:
            if curr_place.bees and place_holder <= self.max_range and place_holder >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        curr_location = self.place
        Insect.reduce_armor(self, amount)
        if self.armor <= 0:
            for bee in list(curr_location.bees):
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        random_bee = random_or_none(bees_in_place)
        if self.digesting == 0 and bees_in_place:
            Insect.reduce_armor(random_bee, random_bee.armor)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.is_real:
            ...
            return Insect.death_callback(self)
        exit = self.place.exit
        if self.is_real:
            while exit != None:
                if exit.ant:
                    for i in gamestate.ants:
                        if exit.ant == i and not exit.ant.buffed:
                            exit.ant.damage = exit.ant.damage * 2
                            exit.ant.buffed = True
                        if isinstance(exit.ant, ContainerAnt) and type(exit.ant.contained_ant) == type(i) and not exit.ant.contained_ant.buffed:
                            exit.ant.contained_ant.damage = exit.ant.contained_ant.damage * 2
                            exit.ant.contained_ant.buffed = True
                exit = exit.exit
            exit = self.place
            while exit != gamestate.beehive:
                if exit.bees:
                    return random_or_none(exit.bees).reduce_armor(self.damage)
                exit = exit.entrance
        ...
        if not self.is_real:
            ...
            while self.place.exit != None and self.place.exit != gamestate.beehive:
                if self.place.ant:
                    if not isinstance(self.place.ant, ContainerAnt):
                        if not self.place.ant.buffed:
                            self.place.ant.damage *= 2
                            self.place.ant.buffed = True
                    if isinstance(self.place.ant, ContainerAnt) and self.place.ant.contained_ant:
                        if not self.place.ant.contained_ant.buffed:
                            self.place.ant.contained_ant.damage *= 2
                            self.place.ant.contained_ant.buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    effect_applied = effect(bee.action, bee)
