...
def accumulate(combiner, base, n, f):
    ...
    16
    """
    k, prevCombined = 1, base
    while k <= n:
        prevCombined = combiner(f(k), prevCombined) #call combiner on base combined
        k += 1
        prevCombined
    return prevCombined
...
def compose1(h, g):
    """Return a function f, such that f(x) = h(g(x)).    def f(x):
            return h(g(x))
        return f"""

    return (lambda f: lambda h: lambda g: x)(x)(g(x))(x)
...
def make_repeater(h, n):
    ...
    def nRepeat(k):
        ...
        while i <= n:
            i += 1
            k = h(k)
        return k #returns h(k) and k is now h(k) n times
    return nRepeat
